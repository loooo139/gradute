                ...
                for con in soup_contents:
                    ...
                    if img is not None:
                        ...
                        if len(text) > 0:
                            contents.append(['p',text])
                return {'title':title, 'datetime':datetime, 'content':contents}
            elif 'allData' in html_cont:
                test = re.findall('(var allData = )(\{.+?\};)', html_cont)[0][1]
                test=test[:-1]
                content=json.loads(test)
                mainContent=content['docData']['contentData']['contentList'][0]['data']
                soup = BeautifulSoup(mainContent, 'html.parser')
                contents = []
                soup_contents = soup.find_all('p')
                for con in soup_contents:
                    img = con.find('img')
                    # 图片
                    # a = con.get('style',None)
                    ...
                    # 正文
                        ...
                        if len(text) > 0:
                            contents.append(['p', text])
                return {'title':content['docData']['title'],'datetime':content['docData']['newsTime'],'content':contents}
